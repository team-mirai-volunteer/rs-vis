      ...
      <div className="shrink-0 border-b border-border px-3 py-2.5">
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-center gap-1.5">
            <p className="truncate text-base font-bold text-mirai-text">{row.sectionName}</p>
...
function HistoryTab({
  ...
  const columns: GridColumn<HistoryRow>[] = [
    ...
    },
    {
      ...
      render: r => <span className="text-mirai-text">{formatYen(r.row.amount)}</span>,
    },
    {
      key: 'rate',
      label: '増減率',
...
function JikouTab({
  ...
  if (error) return <p className="p-3 text-destructive">取得に失敗しました: {error}</p>;
  if (loading || !detail) return <p className="p-3 text-mirai-text-muted">読み込み中…</p>;
  ...
  const columns: GridColumn<MOFJikouItem>[] = [
      ...
      render: it => (
        <span className="flex w-full min-w-0 items-center gap-1">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="min-w-0 truncate text-mirai-text-secondary underline underline-offset-4 hover:text-primary-accent"
          >
            {it.name}
          </a>
          {it.description && (
            <Button
              ...
              className="ml-auto size-5 shrink-0 text-mirai-text-muted hover:bg-transparent hover:text-primary-accent"
            >
    ...
    },
    {
      ...
      render: it => <span className="text-mirai-text">{formatYen(it.amount)}</span>,
    },
    {
      key: 'previousAmount',
      label: '前年度額',
      ...
      {descriptionItem && (
        ...
        >
          <div
            ...
            tabIndex={-1}
            className="mx-4 max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-mirai-border bg-card p-4 shadow-soft outline-none"
            onClick={e => e.stopPropagation()}
          >
            <div className="mb-2 flex items-start justify-between gap-3">
              <h3 className="text-sm font-bold text-mirai-text">{descriptionItem.name}</h3>
              <Button
                variant="ghost"
                size="icon-sm"
                aria-label="閉じる"
            ...
            <p className="whitespace-pre-wrap text-xs leading-relaxed text-mirai-text-secondary">
