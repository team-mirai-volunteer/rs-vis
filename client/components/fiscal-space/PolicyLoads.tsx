    ...
    {basis ? <>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {([
          ...
          ['annualOperatingGwh', '稼働後の年間電力（ピーク不明時）', 0, 1e6, 1, 'GWh/年'],
          ...
          label={`${policy.name}・${label}`} value={basis[key]} min={min} max={max} step={step} unit={unit}
      ...
      <p className="text-sm">人員の稼働率増分＝追加人年÷当該産業の年間人員能力。設備の稼働率とは別の代理指標です。
        ...
        直接入力した稼働後ピークがある場合はそちらを使用し、年間電力から重ねて加算しません。</p>
      ...
      {([
        ...
        label={`${policy.name}・${label}`} value={value[key]} max={max} step={step} unit={unit}
