...
export function ElectricityBaseline({ baseline, projection }: {
  baseline: Simulation; projection: Simulation;
}) {
  return <Card><CardHeader><h2 className="text-lg font-bold">全政策に共通する電力需要・火力燃料の経路</h2>
    ...
    <p>電力需要・燃料単価・政策別の負荷は「詳細な条件」の「産業・電力負荷の条件」で変更できます。</p>
...
export function ElectricitySettings({ value, onChange, estimated }: {
  value: ElectricityBaselineCase; onChange: (v: ElectricityBaselineCase) => void; estimated: boolean;
}) {
  return <section className="space-y-3 text-xs" aria-labelledby="electricity-settings-title">
    <h3 id="electricity-settings-title" className="text-base font-bold">全政策に共通する電力需要・火力燃料の経路</h3>
    <p>電力需要と火力燃料の条件は、政策なしの場合を含む全政策に適用します。既定計画と政策欄には同じ事業を重ねて入力しないでください。</p>
    {estimated && <p>概算モードのピーク需要には地域別の年度見通しを使用します。下のピーク増加率を変更すると、既定の年0.4%からの差を見通しに累積して反映します。年間電力量と火力燃料は下の共通経路で別に計算します。</p>}
