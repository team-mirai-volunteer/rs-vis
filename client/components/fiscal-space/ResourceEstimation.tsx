...
export function ResourceEstimation({ result, value }: {
  result: FiscalCalculation; value: ResourceAssumptions;
}) {
  ...
  return <section aria-label="産業・電力負荷の概算" className="space-y-3 rounded-xl border border-mirai-border bg-card p-5">
    <h2 className="text-lg font-bold">産業・電力制約の概算</h2>
    <p className="text-xs">計算済みの評価方法：{value.mode === 'estimated' ? '公表統計を基に概算' : '手入力した負荷だけで評価'}。概算負荷 {value.loadScale}倍、消費に回る割合 {percent(value.spendingShare, 0)}。</p>
...
export function ResourceSettings({ value, onChange }: { value: ResourceAssumptions; onChange: (v: ResourceAssumptions) => void }) {
