...
export function Demographics({ steps, baseline, longRun, value, baseYear }: {
  ...
  return <Card><CardHeader>
    ...
    <p className="text-xs leading-relaxed">主表示は{FERTILITY_LABELS[steps[0]?.demographics?.fertilityVariant ?? value.fertilityVariant]}です。出生数と年齢別人口も同じ推計にそろえています。出生率は外国人を含む総人口ベースで、厚労省の日本人ベースの実績とは定義が異なります。低位の採用はシミュレータの仮定で、実現確率を示すものではありません。</p>
    ...
    {shortRows.length > 0 && <div className="overflow-x-auto" tabIndex={0} role="region" aria-label="人口動態の経路表"><table className="w-full min-w-[900px] text-right text-sm tabular-nums">
      <caption className="text-left text-xs">指数は年0（{baseYear}年）＝1。各推計の同じ基準年で正規化。出生率・出生数・労働力・税収は政策込み。</caption>
      ...
      <tbody>{shortRows.map(({ step, base }) => {
        ...
        return <tr key={step.state.year} className="border-t border-mirai-border">
          ...
          <td>{d.tfr.toFixed(3)}</td>
          <td>{index(d.labourForceIndex)}</td>
          <td>{index(d.population65Index)}</td>
          <td>{index(d.childIndex)}</td>
          <td>{people(step.state.labour.labourForce)}</td>
          <td>{people(base.state.labour.labourForce)}</td>
          <td>{people(d.births)}</td>
          <td>{money(step.state.fiscal.taxRevenue)}</td>
          <td>{`${Math.round(d.cumulativeExtraBirths).toLocaleString('ja-JP')}人`}</td>
        </tr>;
      })}
      {longRows.map(r => { const d = r.demographics; return <tr key={`long-${r.year}`} className="border-t border-mirai-border">
        <th scope="row" className="p-2">{r.year}（長期）</th><td>{baseYear + r.year}{r.beyondProjection && '†'}</td>
        <td>{d.tfr.toFixed(3)}</td>
        <td>{index(r.labourForceIndex)}</td>
        <td>{index(d.population65Index)}</td>
        <td>{index(d.childIndex)}</td>
        <td>—</td><td>—</td>
        <td>{people(d.births)}</td>
        <td>{money(r.taxRevenue)}</td>
        <td>{`${Math.round(r.extraBirths).toLocaleString('ja-JP')}人`}</td>
      </tr>; })}</tbody>
    </table></div>}
