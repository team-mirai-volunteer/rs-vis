...
export function ModelSensitivity({ rows, horizon, initial }: ModelSensitivityProps) {
  return <section className="space-y-4 border-t border-mirai-border pt-5" aria-label="参考上限の感度">
    ...
    <p className="text-xs">投入指数は直接設定または参考校正の値です。1.08は潜在GDPを基準とする投入の1.08倍。参考校正では実質GDPに対する余力をこの基準へ換算します。詳細な条件の初期投入指数を直接変更すると直接設定へ戻ります。コブ＝ダグラスは中間財を含まない既存の定式化で、モデル間の違いには投入範囲も含みます。探索は代表消費税率がゼロになる額や、本人・事業主への配分に応じた社会保険料収入の限度でも止まります。</p>
    <p className="text-xs">通常稼働は初期投入の構成を保った比例利用と仮定し、各モデルで初期潜在GDPに一致させます。政策は投入量を変え、通常・最大能力を同じ関数で再計算します。研究・OECD方式の学力改善はTFP、旧教育方式・保育・労働供給は有効労働、公共資本は公共サービスによる生産性と設備量、産業事業は設備、発電・系統の燃料節約は有効エネルギーへ換算します。公共資本の設備量経路と事業の供給係数は、設備0.35・エネルギー0.15という固定の参照弾力性で投入指数へ変換する仮定です。選択モデルの弾力性へ付け替えて効果を固定することはしません。発電の確実供給GWが設定されていれば、その増加率で有効エネルギー増分を制限します。未設定なら燃料節約由来の換算にとどまります。産業人員・電力ピークの制約は別途評価します。</p>
...
export function LongRun({ rows, value }: {
  rows: FiscalCalculation['longRun']; value: LongRunAssumptions;
}) {
  return <section id="long-run" className="scroll-mt-20 space-y-4 rounded-xl border border-mirai-border bg-card p-5" aria-label="長期シナリオ">
    ...
    <p className="text-sm">公表乗数の期間後は、設定した成長率・物価・借換金利・便益実現率で条件付き計算します。公表モデルによる予測でも、長期の財政上限でもありません。恒久政策の費用は毎年残り、投資便益は稼働時期・耐用年数・減耗に従います。名目年額は固定し、短期終了後の各支出年の期首価格に長期物価を反映して購入量を計算します。過去に支出した投資の量は変えません。</p>
    <p className="text-xs">計算済みの条件：実質成長率 {percent(value.realGrowth)}、物価 {percent(value.inflation)}、借換金利 {percent(value.rate)}、便益実現率 {percent(value.realization, 0)}。</p>
...
export function ProductionSettings({ controlInputs, controlParameters, controlInflation, onParameters, onInputs, onInflation }: {
...
export function LongRunSettings({ value, onChange }: { value: LongRunAssumptions; onChange: (v: LongRunAssumptions) => void }) {
