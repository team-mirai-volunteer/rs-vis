...
export function LongRun({ rows, value, onChange }: {
  ...
  return <section id="long-run" className="scroll-mt-20 space-y-4 rounded-xl border border-mirai-border bg-card p-5" aria-label="長期シナリオ">
    ...
    <p className="text-xs">長期の年数は評価期間の終了後に最低5年を確保するため、評価期間15年では{Math.max(value.years, 20)}年まで計算します。短期末のGDP・価格の乖離は5年で解消する仮定。CPIの乖離による既存歳出の連動分も同じ期間で解消します。政策経路の借換金利は上の長期金利に、公表期間末の公表長期金利反応を同じ5年で解消させて加えます。政策なし経路は上の長期金利のままです。供給便益には設定した純追加性に加えて上記実現率を掛けます。金利からGDPへの追加効果、長期の産業・物価制約は未推計です。</p>
  </section>;
}
