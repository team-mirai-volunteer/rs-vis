  ...
  return <section aria-label="財政余力の探索結果" className="space-y-3">
    <Card><CardContent className="space-y-4 pt-5">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div><p>理論上限（年額）</p><p className="text-2xl font-bold">{estimate.status === 'search-cap' ? '≥ ' : ''}{money(estimate.theoreticalMaximum)}</p></div>
        <div><p>留保後の参考上限</p><p className="text-2xl font-bold">{money(estimate.recommendedEnvelope)}</p></div>
      ...
      {approximation && <div className="rounded bg-mirai-surface-warm p-3 text-sm" data-testid="cpi-decomposition">
        <p>物価で決まる額の近似：({percent(riskAudit.cpi.limit, 3)} − 基準{percent(approximation.baseline, 3)}) ÷ {(approximation.slope * 100).toFixed(4)}ポイント/兆円 ≈ {money(approximation.amount)}。</p>
