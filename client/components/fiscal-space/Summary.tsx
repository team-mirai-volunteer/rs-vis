...
export function Summary({ estimate, riskAudit, longRun, medium, ...modelProps }: {
  ...
  return <section id="fiscal-envelope" aria-label="財政余力の探索結果" className="scroll-mt-20 space-y-3">
    <Card><CardContent className="space-y-4 pt-5">
      ...
      {medium && <p className="text-xs text-mirai-text-subtle" data-testid="medium-envelope-summary">出生中位 {medium.estimate.status === 'unevaluated' ? '算出不可' : `${money(medium.estimate.recommendedEnvelope, 1)} / 年`}（同じ配分・ストレス条件）</p>}
