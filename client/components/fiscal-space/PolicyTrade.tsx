...
import { POLICY_TRADE_CHANNELS, POWER_SOURCE, POWER_DETAIL_SOURCE, POWER_FIRM_NOTE, SEMICONDUCTOR_SOURCE, PROJECT_IMPORT_REFERENCE, PROJECT_IMPORT_NOTE, RESEARCH_PROJECT_SOURCE, RESEARCH_PROJECT_NOTE,
  industryTrade, industryImportBreakEven, INDUSTRY_TRADE_REFERENCE, INDUSTRY_TRADE_REFERENCE_PERCENT, powerTrade, SEMICONDUCTOR_FINANCIAL_SOURCE, type IndustryTradeCase } from '@/app/lib/fiscal-space/policy-trade';
...
export function PolicyTrade({ policies, value, onChange, settings = false, onPowerSettings }: { policies: Policy[]; value: TradeForm; onChange?: (v: TradeForm) => void; settings?: boolean; onPowerSettings?: () => void }) {
  ...
  return <Card><CardHeader><h2 className="text-lg font-bold">政策ごとの輸出・国内代替</h2><p className="text-sm leading-relaxed">半導体の海外販売、発電の燃料輸入削減、減税による競争力は別の経路です。公表マクロモデルの共通反応だけでは分野固有の便益を比較できません。</p></CardHeader><CardContent className="space-y-4 text-xs leading-relaxed">
    {!settings && <div role="region" aria-label="政策別の貿易経路" className="overflow-x-auto" tabIndex={0}>
    ...
    <details open={settings || undefined}><summary className="cursor-pointer text-sm font-bold">{settings ? '政策別の事業条件' : '政策別の輸出入試算の内訳'}</summary><div className="mt-4 space-y-4">
      ...
      {isPower ? <>
        ...
        {settings && <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {selected.id === 'rd' && <p data-testid="research-project-estimate">{RESEARCH_PROJECT_NOTE} <a className="underline" href={RESEARCH_PROJECT_SOURCE} target="_blank" rel="noreferrer">収益率の参考：RIETI</a></p>}
    ...
    <details><summary className="cursor-pointer font-bold">本体への反映方法と推計の限界</summary><div className="mt-2 space-y-2">
      <p>建設時は、公表モデルの初年度輸入反応を基準に、輸入割合を変更した後の国内支出に比例してGDP・物価・雇用等の反応を補正します。元の輸入反応には間接需要も含まれるため、直接調達割合の厳密な推定ではありません。建設時輸入の初期値は全国の固定資本形成からの概算で、部門別校正は未完了です。半導体の稼働時輸入原価は2020年の電子デバイス部門から推計しますが、政府支出の共通反応には引き続き代理モデルとしての不確実性が残ります。</p>
