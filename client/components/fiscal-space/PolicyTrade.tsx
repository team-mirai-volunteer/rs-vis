...
import { POLICY_TRADE_CHANNELS, POWER_SOURCE, POWER_DETAIL_SOURCE, POWER_FIRM_NOTE, SEMICONDUCTOR_SOURCE,
...
export function PolicyTrade({ policies, value, onChange, settings = false, onOpenSettings, onPowerSettings }: { policies: Policy[]; value: TradeForm; onChange?: (v: TradeForm) => void; settings?: boolean; onOpenSettings?: () => void; onPowerSettings?: () => void }) {
  ...
  return <Card><CardHeader><h2 className="text-lg font-bold">政策ごとの輸出・国内代替</h2><p className="text-sm leading-relaxed">半導体の海外販売、発電の燃料輸入削減、減税による競争力は別の経路です。公表マクロモデルの共通反応だけでは分野固有の便益を比較できません。</p></CardHeader><CardContent className="space-y-4 text-xs leading-relaxed">
    ...
    {!settings && <button type="button" className="text-sm text-primary-accent underline" onClick={onOpenSettings}>政策別の事業条件を設定</button>}
    <details open={settings || undefined}><summary className="cursor-pointer text-sm font-bold">政策別の事業条件から試算する</summary><div className="mt-4 space-y-4">
