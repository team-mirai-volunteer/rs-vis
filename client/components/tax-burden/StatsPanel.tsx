  ...
  if (axis === 'income' || !age) {
    rows = data.deciles.map(d => {
      ...
      return { key: `d${d.decile}`, title: `第${d.decile}十分位`, subtitle: `${bound(d.lowerBound, d.upperBound)}・平均${man(d.annualIncome)}`, denominator: d.annualIncome, denominatorLabel: '年間収入',
        parts, total, rate: total / d.annualIncome, householdSize: d.householdSize, headAge: d.headAge,
    ...
    rows = group.classes.map(c => {
      ...
      return { key: c.label, title: c.label, subtitle: `実収入 ${man(c.realIncomeAnnual)}${c.pensionBenefitAnnual > c.salaryAnnual ? `（うち年金 ${man(c.pensionBenefitAnnual)}）` : ''}`, denominator: c.realIncomeAnnual, denominatorLabel: '実収入',
        parts, total, rate: total / c.realIncomeAnnual, householdSize: c.householdSize, headAge: c.headAge,
        extra: [{ label: '公的年金給付', value: mode === 'rate' ? pct(c.pensionBenefitAnnual / c.realIncomeAnnual) : yen(c.pensionBenefitAnnual) }, { label: '可処分所得', value: yen(c.disposableAnnual) }] };
    });
    ...
    denominatorNote = `${group.population}。年齢階級別の年間収入は非公表のため、分母は実収入（勤め先収入・公的年金給付等の年額換算）。`;
  }
    ...
    <Card><CardHeader className="gap-3">
      ...
      <CardContent>
        <div className="space-y-2">{rows.map(r => <div key={r.key} className="grid items-center gap-2 text-xs sm:grid-cols-[170px_minmax(0,1fr)_90px]">
          ...
          <div className="flex h-6 w-full overflow-hidden rounded-md bg-mirai-surface" role="img" aria-label={`${r.title} ${mode === 'rate' ? `負担率${pct(r.rate)}` : `負担額${yen(r.total)}`}`}>
          ...
          <div className="text-right font-bold tabular-nums">{mode === 'rate' ? pct(r.rate) : man(r.total)}</div>
        </div>)}</div>
