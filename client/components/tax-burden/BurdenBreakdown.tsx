...
export function BurdenBreakdown({ before, after, impact, includeConsumption, pensionIncome, title }: {
  ...
  const total = (r: BurdenResult) => r.netBurden + (includeConsumption ? r.consumptionTax : 0);
  return <div className="grid gap-5 xl:grid-cols-2">
    <Card><CardHeader><h2 className="font-bold">{title ?? '選んだ年収の負担内訳'}</h2><p className="text-xs text-mirai-text-secondary">総収入 {yen(before.income)}{pensionIncome ? `（うち公的年金 ${yen(pensionIncome)}）` : ''} ／ 年額・世帯単位</p></CardHeader>
      <CardContent><div className="overflow-x-auto"><table className="w-full text-sm tabular-nums">
        ...
        <tfoot><tr className="font-bold"><th scope="row" className="pt-3 text-left">純負担額{includeConsumption ? '（消費税込み）' : ''}</th><td className="pt-3 text-right">{yen(total(before))}</td>{after && <td className="pl-3 pt-3 text-right">{yen(total(after))}</td>}</tr></tfoot>
