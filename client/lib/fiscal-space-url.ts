...
export const FISCAL_MODEL_VERSION = '2026-09-21.2';
...
export function decodeScenarioDetailed(hash: string): { form: FiscalForm } & ScenarioRestore {
  ...
  if (!payload || typeof payload !== 'object' || !('version' in payload) || ![FISCAL_MODEL_VERSION, '2026-09-21.1', '2026-09-20.1', '2026-09-17.1', '2026-09-16.6', '2026-09-16.5', '2026-09-16.4', '2026-09-16.3', '2026-09-16.2', '2026-09-16.1', '2026-09-15.8', '2026-09-15.7', '2026-09-15.6', '2026-09-15.5', '2026-09-15.4', '2026-09-15.3', '2026-09-15.2'].includes(String(payload.version)) || !('form' in payload)) throw new Error('Unsupported model version');
  if (payload.version !== FISCAL_MODEL_VERSION && payload.form && typeof payload.form === 'object' && 'calibration' in payload.form) {
    if (!Object.hasOwn(payload.form, 'capacity')) {
      ...
      filled.push('capacity（最大GDPの参考校正：旧リンクは投入指数の直接設定を維持）');
    }
