...
import { INDUSTRY_CASE, SEMICONDUCTOR_CASE, RESEARCH_CASE, powerCase } from '@/app/lib/fiscal-space/policy-trade';
...
import { optimizationDefaults } from './fiscal-objective';

export const defaults = (dataset: JapanDataset = 'latest') => {
  const initial = initialEconomy(dataset);
  return { dataset, horizon: 5, optimization: optimizationDefaults(), capacity: { ...CAPACITY_DEFAULTS }, inputs: { ...initial.production.inputs }, longRun: { ...LONG_RUN }, loads: {} as Record<string, Policy['load']>, resource: { ...RESOURCE_DEFAULTS }, corporateShare: CORPORATE_WAGE_SHARE,
  supply: Object.fromEntries(Object.entries(SUPPLY_CASES).map(([id, ref]) => [id, { ...ref.settings }])),
  trade: { selected: 'semiconductors', industry: Object.fromEntries(POLICIES.map(policy => [policy.id, { ...(policy.id === 'semiconductors' ? SEMICONDUCTOR_CASE : policy.id === 'rd' ? RESEARCH_CASE : INDUSTRY_CASE) }])), power: powerCase('solar') } as TradeForm,
