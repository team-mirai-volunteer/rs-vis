"""
事業別 支出先データ品質スコア — AI評価フェーズ（Google AI Studio 検証版）

⚠️ これは検証/深掘り用の派生スクリプト。本番は scripts/score-project-quality-ai.py（OpenRouter版）。
   採点ロジック（プロンプト・レベル定義・金額加重集計・軸C/D機械計算・総合重み・出力スキーマ）は
   本番と完全一致を保つこと。本ファイルの差分は API 接続まわり（キー/base-url/model/thinking/
   aiSourceラベル/キャッシュ名）に限定する。本番側を変更したら本ファイルにも同期すること。
   設計: docs/quality-scoring-guide.md「AI評価スクリプトの構成（本番版と Google 検証版）」
...
出力:
  public/data/project-quality-scores-{year}.json           新スキーマで上書き（aiSource=google:<model>）

実行:
  GOOGLE_API_KEY=... python3 scripts/score-project-quality-ai-google.py [--year 2024] \\
                     [--model gemini-2.5-flash] [--limit N] [--concurrency 2]

  AI判定は Google AI Studio の OpenAI互換エンドポイント経由（実 Gemini）。
  GOOGLE_API_KEY が未設定なら決定的ヒューリスティックで完走し aiSource="heuristic"。
  実Gemini時は aiSource="google:<model>"。

  ※ Google無料枠は RPM/TPM/RPD 制限あり。全件は RPD を跨ぐ前提で、永続キャッシュにより
     日跨ぎ分割再開が可能（成功分のみ永続化、heuristic補完は非永続で翌回に実Geminiで穴埋め）。
     429 が続く場合は --concurrency を下げる。
  ※ Google の OpenAI互換層は OpenRouter の reasoning フィールド非対応のため送らない（§make_client/llm_json）。
  ※ 確定スコア（2025と基準一致）は本番 OpenRouter版 + 有料 gemini-3.5-flash。本スクリプトは検証/プレビュー用。
...
parser.add_argument('--model', default='gemini-2.5-flash',
                    help='Geminiモデル (デフォルト: gemini-2.5-flash。例: gemini-2.5-flash-lite)。Googleネイティブ表記（google/接頭辞なし）')
parser.add_argument('--base-url', default='https://generativelanguage.googleapis.com/v1beta/openai/',
                    help='OpenAI互換APIのベースURL (デフォルト: Google AI Studio)')
...
parser.add_argument('--concurrency', type=int, default=2,
                    help='並列API呼び出し数 (デフォルト2。Google無料枠のレート制限に配慮。429が続けば1に)')
...
# キャッシュは google- 接頭辞で分離（OpenRouter版の同名geminiキャッシュとの混在を防ぐ）。
MODEL_SLUG          = re.sub(r'[^A-Za-z0-9._-]', '-', MODEL.split('/')[-1])
CACHE_FILE          = CACHE_DIR / f'ai-quality-cache-google-{MODEL_SLUG}.json'
...
def make_client():
    ...
    return OpenAI(base_url=args.base_url, api_key=key)


def llm_json(client, model, system, prompt):
    """Google AI Studio(OpenAI互換)で JSON を生成して dict にパース。"""
    # Google の OpenAI互換層は OpenRouter の `reasoning` フィールド非対応（400）のため送らない。
    # gemini-2.5-flash は動的thinkingで判定（無料枠のためコスト問題なし）。
...
def main():
    ...
    if api_key:
        try:
            ...
            print('  [error] openai 未インストール。pip install openai 後に再実行してください。',
                  file=sys.stderr)
            sys.exit(1)
    ai_source = f'google:{MODEL}' if api_key else 'heuristic'
    ...
    if uniques and client:
        ...
        if uniques and not api_key:
            print('  [info] GOOGLE_API_KEY 未設定 → ヒューリスティック判定（aiSource=heuristic）')
