...
| `/sankey-svg` | SVGベース直接支出サンキー図（2024/2025年度・TopNスライダー・予算/支出フィルタ・会計区分フィルタ・府省庁フィルタ複数選択・ノードPin・ミニマップ・検索・支出先なしノード対応・省庁クリック連動フィルタ・検索/フィルタ独立パネル・フィルタ解除ボタン・左下フォントサイズ設定・ズーム連動ラベル拡大・事業列の予算/支出ツールチップ・サイドパネルに会計内訳表示） |
| `/sankey2` | 事前計算レイアウトのCanvas描画サンキー図（Treemapクラスタ配置） |
| `/subcontracts` | 再委託構造ブラウザ（2024/2025年度、カード型フロー図・リスト型サイドパネル・別財源ブロック可視化・会計区分表示・一覧/詳細UIリニューアル・フィルタパネル） |
...
│   └── components/
├── components/               # 共通UIコンポーネント（filters/, subcontracts/）
