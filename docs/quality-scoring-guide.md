...
> **⚠️ 2026-06 改訂: AI評価による新4軸へ移行**
> 最終スコアは **AI評価2軸＋機械計算2軸** で確定する。算出は2フェーズ:
...
>
> AI評価の粒度・根拠・納得感を反復検証する固定30事業セット:
> `docs/quality-evaluation-benchmark.md`
>
> ### スコア軸
> | 軸 | 種別 | 重み | 測るもの |
> |----|------|------|----------|
> | A 支出先の特定可能性 | AI判定 | 28% | 支出先が誰で第三者が実在を確認できるか（名称・法人番号有無・契約概要を総合） |
> | B 使途の説明性 | AI判定 | 22% | 役割・契約概要から「何にいくら使ったか」が理解・検証できるか |
> | C 収支の整合性 | 機械計算 | 15% | 執行額と実支出が許容バンド内で一致するか（10%まで満点） |
> | E 有効性／成果設計の明確さ | AI判定 | 35% | 目的・現状課題・概要から国民生活への寄与が明確・妥当に説明されているか（**0-10の11段階・意図ベース・実測成果ではない**） |
> | D 構造の整合性 | 機械計算 | （参考） | ブロック金額の内部整合・孤立ブロック。**平均99.9でほぼ弁別しないため総合には不算入**・参考表示のみ |
>
> A・B・E=AI判定（85%）、C=機械（15%）。D（構造）は参考表示のみ。
> 軸A・Bは支出先ごとに identifiability/purpose を 0-3 で評価し金額加重で集計、軸Eは事業テキストを **0-10の11段階**で評価（→ スコア = レベル×10）。
> AI判定は **OpenRouter（OpenAI互換API、`openai` SDK）** 経由。既定モデル `google/gemini-3.5-flash`（`--model anthropic/claude-haiku-4-5` 等に変更可）。`OPENROUTER_API_KEY` 未設定時は決定的ヒューリスティックで採点し `aiSource="heuristic"` を付与（本番は `aiSource="openrouter:<model>"`）。
> コスト制御: 重複排除＋ゲーティング＋並列実行（`--concurrency`）＋永続キャッシュ（`data/cache/`、モデル末尾セグメントで正規化＝同一実モデルはプロバイダ跨ぎで再利用）＋ flash系モデル選択。失敗バッチはヒューリスティック補完され非キャッシュなので、再実行で穴埋め可能。
>
> ---
>
