...
try {
  ...
  await expect(page.getByRole('heading', { name: '同じ配分を拡大した場合の参考上限' })).toBeVisible();
  ...
  await expect(substitution('semiconductors').locator('[data-year="3"]')).toContainText('0.000');
  await expect(substitution('semiconductors').locator('[data-year="5"]')).toContainText('+0.101');
  await expect(substitution('generation').locator('[data-year="5"]')).toContainText('+0.066');
  const mature = id => page.locator(`[data-investment="${id}"]`);
  await expect(mature('semiconductors').locator('td').nth(0)).toContainText('4');
  await expect(mature('semiconductors').locator('td').nth(1)).toContainText('+0.224');
  await expect(mature('rd').locator('td').nth(1)).toContainText('+0.150');
  await expect(mature('grid').locator('td').nth(0)).toContainText('6');
  await expect(mature('grid').locator('td').nth(1)).not.toContainText('0.000');
  const semiPotential = comparison.locator('[data-policy="semiconductors"] [data-metric="potentialGdpEffect"]');
  await expect(semiPotential.locator('[data-year="5"]')).not.toContainText('0.000');
  await expect(comparison.locator('[data-policy="defence"] [data-metric="potentialGdpEffect"]')).toContainText('未推計');
  await expect(comparison.locator('[data-policy="rd"] [data-metric="potentialGdpEffect"] [data-year="5"]')).not.toContainText('0.000');
  ...
  await expect(semiPotential.locator('[data-year="5"]')).toContainText('+0.500');
  await expect(semiPotential.locator('[data-year="1"]')).toContainText('0.000');
  ...
  await expect(powerPotential).toContainText('0.000');
  ...
  await expect(mature('generation').locator('td').nth(1)).not.toContainText('0.000');
