...
try {
  ...
  await expect(page.getByLabel('手取り賃金に対する労働時間の弾力性', { exact: true })).toHaveValue('0.1');
  ...
  for (const metric of ['realGdpEffect', 'inflationPressure', 'exports', 'imports', 'domesticSubstitution', 'tradeBalanceEffect', 'potentialGdpEffect']) {
    ...
    for (const year of [1, 3, 5]) await expect(values.locator(`[data-year="${year}"]`)).toHaveCount(13);
  ...
  await expect(projectionTable.locator('thead th')).toHaveCount(16);
  await expect(projectionTable.locator('tbody details')).toHaveCount(0);
  await expect(page.getByRole('img', { name: /5年推移（兆円）/ }).locator('text').filter({ hasText: '兆円' })).toHaveCount(4);
