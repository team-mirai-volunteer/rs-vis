        ...
        <div className="min-w-0 space-y-5">
          {error ? errorCard(error) : !params || !before || !selected ? loading : <>
            ...
            {state.view === 'age' && lifecycle && <>
              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  { label: `${selectedAge}歳の純負担率`, value: rateText(ageRow ? (state.includeConsumption ? ageRow.netRateWithConsumption : ageRow.netRate) : null), note: ageRow ? `${PHASE_LABEL[ageRow.phase]}・総収入 ${Math.round(ageRow.income / 10000).toLocaleString('ja-JP')}万円` : '' },
              ...
              <Card><CardHeader><h2 className="text-lg font-bold">同じ所得階層の人が、年齢とともにどれだけ負担するか</h2><p className="mt-2 text-xs text-mirai-text-secondary">{household.label}・現役期の世帯年収 {(state.income / 10000).toLocaleString('ja-JP')}万円。60〜64歳は賃金{Math.round(state.continuation * 100)}%で継続雇用、65歳から年金{state.workTo69 ? '（69歳まで就労継続）' : ''}。</p></CardHeader>
                <CardContent><LifecycleChart years={lifecycle} state={state} selectedAge={selectedAge} onSelectAge={setSelectedAge} />
                  <div className="mt-4 overflow-x-auto"><table className="w-full text-xs tabular-nums"><thead><tr className="text-mirai-text-secondary"><th scope="col" className="py-1 text-left">年齢</th><th scope="col" className="text-right">総収入</th><th scope="col" className="text-right">うち年金</th><th scope="col" className="text-right">税</th><th scope="col" className="text-right">保険料</th><th scope="col" className="text-right">給付</th><th scope="col" className="text-right">純負担率</th><th scope="col" className="text-right">可処分所得</th></tr></thead>
                    <tbody>{[30, 40, 50, 60, 64, 65, 70, 75, 80].map(a => lifecycle.find(y => y.ageAt === a)!).map(y => <tr key={y.ageAt} className={`border-t border-mirai-border/30 ${y.ageAt === selectedAge ? 'font-bold' : ''}`}><th scope="row" className="py-1 text-left font-normal">{y.ageAt}歳（{PHASE_LABEL[y.phase]}）</th><td className="text-right">{yen(y.income)}</td><td className="text-right">{yen(y.pensionIncome)}</td><td className="text-right">{yen(y.incomeTax + y.residentTax + (state.includeConsumption ? y.consumptionTax : 0))}</td><td className="text-right">{yen(y.pension + y.health + y.care + y.employment)}</td><td className="text-right">{yen(y.benefits)}</td><td className="text-right">{rateText(state.includeConsumption ? y.netRateWithConsumption : y.netRate)}</td><td className="text-right">{yen(y.disposable - (state.includeConsumption ? y.consumptionTax : 0))}</td></tr>)}</tbody></table></div>
                  <p className="mt-4 text-xs leading-relaxed text-mirai-text-secondary">年金は老齢基礎年金（満額）＋報酬比例部分（平均標準報酬額×5.481/1000×480か月）。65歳以降の医療保険は国民健康保険（東京都特別区の統一保険料・要照合）、75歳から後期高齢者医療（東京都広域連合）、介護保険第1号は国の標準段階×全国平均基準額。年金生活者支援給付金は所得要件を満たす場合のみ。</p>
                </CardContent>
              </Card>
