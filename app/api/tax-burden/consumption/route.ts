...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(readDataJson<ConsumptionDataset>(`tax-burden-consumption-${year}.json`, 'python scripts/generate-tax-burden-stats.py'),
      { headers: { 'Cache-Control': API_CACHE_CONTROL } });
  } catch {
    return NextResponse.json({ error: '消費支出データを読み込めませんでした。' }, { status: 503 });
  }
}
