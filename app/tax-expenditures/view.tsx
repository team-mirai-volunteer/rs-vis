...
export default function TaxExpenditures() {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
      <header>
        <p className="text-sm font-semibold text-primary-accent">税優遇から政策を見る</p>
        <h1 className="mt-2 text-3xl font-bold">租税特別措置(試作)</h1>
        <p className="mt-3 max-w-3xl leading-7 text-mirai-text-secondary">誰に、どんな税優遇が適用されているか。法人税関係の租特を調べ、同じ制度を支える行政事業レビュー（RS）の事業をたどります。</p>
      </header>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <Card className={panel}>
          ...
          <Button onClick={() => { setLinkedOnly(true); setQuery(''); }} className="mt-3" size="sm">RSとの関連を確認する</Button>
        </Card>
        <Card className={panel}>
      ...
      <section aria-labelledby="measures-title" className="space-y-4">
        ...
        <AssessmentTable year={year} ids={new Set(measures.map(m => m.id))} />
        <CreditChart year={year} ids={new Set(measures.map(m => m.id))} onSelect={id => {
