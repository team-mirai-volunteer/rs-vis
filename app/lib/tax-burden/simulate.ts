import type { BurdenResult, ConsumptionDataset, HouseholdId, Reform, TaxParameters, TaxState } from '@/types/tax-burden';
...
export type SimulateState = Pick<TaxState, 'income' | 'household' | 'share' | 'age' | 'bonus'> &
  Partial<Pick<TaxState, 'includeConsumption' | 'consumptionAssumption'>>;
...
export function simulate(state: SimulateState, p: TaxParameters,
  reform: Reform = { ...BASE_REFORM, childMonthly: p.childMonthly }, consumption?: ConsumptionDataset | null): BurdenResult {
  ...
  return { income, salaries, consumptionTax, netRateWithConsumption: income > 0 ? (taxes.netBurden + consumptionTax) / income : null,
    ...
    reformCredit: taxes.reformCredit, grossBurden: taxes.grossBurden, benefits: taxes.benefits, netBurden: taxes.netBurden,
    netRate: income > 0 ? taxes.netBurden / income : null, outOfScope: scopeReasons.length > 0, scopeReasons };
}

export function curveSeries(state: TaxState, p: TaxParameters, id: HouseholdId, reform?: Reform, consumption?: ConsumptionDataset | null): BurdenResult[] {
  return Array.from({ length: 401 }, (_, i) => simulate({ ...state, household: id, income: i === 0 ? 10000 : i * 50000 }, p, reform, consumption));
}
